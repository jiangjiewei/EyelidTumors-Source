"""
AI for eyelid tumors: Discriminating between malignant and benign eyelid tumors using artificial intelligence-Source.
...
parser.add_argument('-num_class', '--num_class', default=2, type=int,
                    metavar='N',
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.fine_tuning:
        ...
        if args.arch.find('alexnet') != -1:
            ...
            model.classifier[6] = nn.Linear(num_ftrs, args.num_class)
        elif args.arch.find('inception_v3') != -1:
            num_ftrs = model.fc.in_features
            num_auxftrs = model.AuxLogits.fc.in_features
            model.fc = nn.Linear(num_ftrs, args.num_class)
            model.AuxLogits.fc =nn.Linear(num_auxftrs,args.num_class)
            ...
            model.classifier = nn.Linear(num_ftrs, args.num_class)
    ...
    if args.distributed:
        ...
        if args.arch.startswith('alexnet-1') or args.arch.startswith('vgg-1'):
            ...
            # print('break dataparallel for two transfer learning')
            model = torch.nn.DataParallel(model).cuda()

    # define loss function (criterion) and optimizer
    # criterion = nn.CrossEntropyLoss().cuda(args.gpu)
    #jiangjiewei add weight for crossentropyloss
    criterion = nn.CrossEntropyLoss(weight=torch.Tensor([1.0, 2.5])).cuda(args.gpu)
    # use_cuda = True
    ...
    if args.arch.find('alexnet') != -1:
        ...
        fine_tune_parameters =model.module.classifier[6].parameters()
    elif args.arch.find('densenet121') != -1:
    ...
    if args.advprop:
        ...
        normalize = transforms.Normalize( mean = [0.58574647, 0.43944123, 0.38220894], std = [0.20514074, 0.17088173, 0.18773124] )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        acc1, loss1 = validate(val_loader, model, criterion, args)

        # remember best acc@1 or loss@1 and save checkpoint
...
def save_checkpoint(state, is_best, is_best_loss, pre_filename='my_checkpoint.pth.tar'):
    check_filename = pre_filename + '_checkpoint.pth.tar'
    des_best_filename = pre_filename + '_model_best.pth.tar'
    des_best_loss_filename = pre_filename + '_model_bestloss.pth.tar'
...
def fundus_test():
    ...
    dataset_subdir = ['test1',  
	                  'external_cropped_0812'
				      ]
    model_names = ['densenet121',
                    # 'vgg16',
                   # 'inception_v3',
    ...
    for index_name in range(0,len(model_names)):
        ...
        for i in range(0,len(dataset_subdir)):
            dataset_dir = './data/' + dataset_subdir[i]
            resultset_dir = './result_tumor_cut_1_2.5_test12/' + dataset_subdir[i]
            args, model, val_transforms = load_modle_trained(args)
            mk_result_dir(args, resultset_dir)
            eye_tumor_test_exec(args, model, val_transforms, dataset_dir,resultset_dir)
...
def load_modle_trained(args):
    ...
    # PATH = pre_name + '_model_bestloss.pth.tar'
    # PATH = './best.ckpt'
...
def mk_result_dir(args,testdata_dir='./data/val1'):
    ...
    grade1_grade2 = 'benign_malignancy'
    grade2_grade1 = 'malignancy_benign'
...
def eye_tumor_test_exec(args,model,val_transforms,testdata_dir='./data/val1',resultset_dir='./data/val1'):
    # switch to evaluate mode
    ...
    grade1_grade1 = 'benign_benign'
    grade2_grade2 = 'malignancy_malignancy'
    ...
    with torch.no_grad():
        ...
        for img in img_list:
            ...
            # prob_adjust = prob + torch.tensor([-0.1,0.1]).cuda(args.gpu)
            prob_adjust = prob
            pred = torch.argmax(prob_adjust, dim=1)
            pred = pred.cpu().numpy()
            pred_0 =  pred[0]
            ...
            if pred_0 == 0:
                ...
                file_new_1 = desdatadir + '/benign_malignancy' + '/' + img
